...
router.post('/register', async (req, res) => {
  try {
    ...
    return res
      ...
      .json({ success: false, error: 'register_failed', message })
  }
})
...
router.post('/login', async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'login_failed', message: 'Internal server error' })
  }
})
...
router.post('/logout', authenticateClient, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'logout_failed', message: 'Internal server error' })
  }
})
...
router.get('/profile', authenticateClient, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: 'profile_failed', message: 'Internal server error' })
  }
})
...
router.post('/refresh', authenticateClient, async (req, res) => {
  try {
    ...
    if (!ok) {
      return res
        .status(401)
        .json({ success: false, error: 'invalid_session', message: 'Invalid or expired session' })
    }
    ...
    return res
      .status(500)
      .json({ success: false, error: 'refresh_failed', message: 'Internal server error' })
  }
})
...
router.post('/redeem', authenticateClient, async (req, res) => {
  try {
    ...
    if (!code || typeof code !== 'string') {
      return res
        .status(400)
        .json({ success: false, error: 'invalid_code', message: '兑换码不能为空' })
    }
    ...
    return res
      .status(400)
      .json({ success: false, error: 'redeem_failed', message: error.message || '兑换失败' })
  }
})

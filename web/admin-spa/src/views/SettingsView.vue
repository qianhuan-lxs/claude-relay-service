        ...
        <div v-show="activeSection === 'data'">
          <div class="space-y-6">
            ...
            <div class="rounded-lg bg-white/80 p-6 shadow-lg backdrop-blur-sm dark:bg-gray-800/80">
                ...
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <div v-if="exportPreview.apikeys !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">
                      {{ exportPreview.apikeys }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">API Keys</div>
                  </div>
                  <div v-if="exportPreview.accounts !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-green-600 dark:text-green-400">
                      {{ exportPreview.accounts }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">账户</div>
                  </div>
                  <div v-if="exportPreview.users !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">
                      {{ exportPreview.users }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">用户</div>
                  </div>
                  <div v-if="exportPreview.plans !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-orange-600 dark:text-orange-400">
                      {{ exportPreview.plans }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">套餐</div>
                  </div>
                  <div v-if="exportPreview.orders !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-red-600 dark:text-red-400">
                      {{ exportPreview.orders }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">订单</div>
                  </div>
                  <div v-if="exportPreview.announcements !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">
                      {{ exportPreview.announcements }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">公告</div>
                  </div>
                  <div v-if="exportPreview.tutorials !== undefined" class="text-center">
                    <div class="text-2xl font-bold text-teal-600 dark:text-teal-400">
                      {{ exportPreview.tutorials }}
                    </div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">教程</div>
                  </div>
                </div>
              </div>
                ...
                <div class="flex items-center space-x-4">
                  <label class="flex cursor-pointer items-center">
                    <input
                      ...
                      type="checkbox"
                    />
                    <span class="text-sm text-gray-700 dark:text-gray-300">包含使用统计数据</span>
                  </label>
                  <label class="flex cursor-pointer items-center">
                    <input
                      v-model="exportOptions.decrypt"
                      class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      type="checkbox"
                    />
              ...
              <!-- 导出按钮 -->
              <button
                class="w-full rounded-lg bg-blue-600 px-6 py-3 text-white shadow-lg transition-all hover:bg-blue-700 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div class="rounded-lg bg-white/80 p-6 shadow-lg backdrop-blur-sm dark:bg-gray-800/80">
              ...
              <div class="mb-4">
                ...
                <input
                  ...
                  class="block w-full text-sm text-gray-500 file:mr-4 file:rounded-lg file:border-0 file:bg-blue-50 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/50 dark:file:text-blue-300"
                  type="file"
              ...
              <div class="mb-4 space-y-3">
                <label class="flex cursor-pointer items-center">
                  <input
                    v-model="importOptions.force"
                    class="mr-2 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                    type="checkbox"
                  />
                  <span class="text-sm text-gray-700 dark:text-gray-300">强制覆盖已存在的数据</span>
                </label>
                <label class="flex cursor-pointer items-center">
                  <input
                    ...
                    type="checkbox"
                  />
                  <span class="text-sm text-gray-700 dark:text-gray-300">跳过冲突的数据</span>
                </label>
              </div>

              <!-- 导入按钮 -->
              <button
                class="w-full rounded-lg bg-green-600 px-6 py-3 text-white shadow-lg transition-all hover:bg-green-700 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
              ...
              >
                ...
                <div v-if="importResult.stats" class="text-sm text-gray-700 dark:text-gray-300">
                  <p>已导入: {{ importResult.stats.imported }}</p>
                  <p>已跳过: {{ importResult.stats.skipped }}</p>
                  <p v-if="importResult.stats.errors > 0" class="text-red-600 dark:text-red-400">
                    错误: {{ importResult.stats.errors }}
                  </p>
                </div>
                <p v-if="importResult.message" class="mt-2 text-sm">{{ importResult.message }}</p>
              </div>
            </div>
          </div>
        </div>
...
const selectedExportTypes = ref(['all'])
...
const exportTypes = [
  ...
  { value: 'apiKeyTemplates', label: 'API Key 模板' },
...
const loadExportPreview = async () => {
  ...
  try {
    const response = await apiClient.get('/admin/data/export/preview', {
...
const handleExport = async () => {
  ...
  try {
    ...
    const API_PREFIX = (await import('@/config/api')).API_PREFIX || '/webapi'
    const fullUrl = `${API_PREFIX}/admin/data/export`
    ...
    const response = await fetch(fullUrl, {
      ...
      body: JSON.stringify({
        ...
        decrypt: exportOptions.value.decrypt
      }),
      signal: abortController.value.signal
    })
...
const handleImport = async () => {
  ...
  try {
    const token = localStorage.getItem('authToken')
    const API_PREFIX = (await import('@/config/api')).API_PREFIX || '/webapi'
    const fullUrl = `${API_PREFIX}/admin/data/import`

    const formData = new FormData()
    formData.append('file', selectedFile.value)
    formData.append('force', importOptions.value.force)
    formData.append('skipConflicts', importOptions.value.skipConflicts)
    formData.append('types', JSON.stringify(['all']))
    ...
    if (data.success && isMounted.value) {
      importResult.value = {
        success: true,
        stats: data.stats,
...
onMounted(async () => {
  try {
    ...
    if (activeSection.value === 'webhook') {
      await loadWebhookConfig()
    } else if (activeSection.value === 'data') {
      await loadExportPreview()
    }
  } catch (error) {
